...
class Pipeline:
    ...
    def parse_source(self, output_file, option):
        ...
        if os.path.exists(path) and option is 'existing':
            ...
            source = pd.read_pickle(self.root + 'ast.pkl')
    ...
    def split_data(self):
        ...
        data = data.sample(frac=1, random_state=250)
    ...
    def split_data_no_ratio(self):
        ...
        train_split = int(ratios[0] / sum(ratios) * data_num)
        val_split = train_split + int(ratios[1] / sum(ratios) * data_num)
        data = data.sample(frac=1, random_state=888)
        train = data.iloc[:train_split]
        dev = data.iloc[train_split:val_split]
        # test = data.iloc[val_split:]
        test = dev
    ...
    def dictionary_and_embedding(self, input_file, size):
        ...
        from utils.astnn_util.prepare_data import get_sequences
        ...
        w2v.save(self.root+'train/embedding/node_w2v_' + str(size))  #保存训练后的模型

    # generate block sequences with index representations
    def generate_block_seqs(self,data_path,part):
        from utils.astnn_util.prepare_data import get_blocks as func
        from gensim.models.word2vec import Word2Vec

        word2vec = Word2Vec.load(self.root+'train/embedding/node_w2v_' + str(self.size)).wv  #加载训练后的模型
        # word2vec = Word2Vec.load(self.root + 'train/embedding/node_w2v_128').wv
