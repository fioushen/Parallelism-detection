...
class DGCNN:
    ...
    def gcnn_layer(self, input_Z, in_dim, out_dim, layer_id):
        ...
        with tf.name_scope(f"gcnn_layer_{layer_id}"):
            ...
            ###################################################################################
            ####添加注意力机制
            # A_dim = self.ajacent.dense_shape[0]
            # A_dim = tf.convert_to_tensor(A_dim, dtype=tf.float32)
            # B_dim = A_dim
            # WZ = tf.random_uniform((self.k, self.k), dtype=tf.float32)
            # print(A_dim)
            # WZ = tf.Variable(WZ, name=f"dgcnn_WZ_{layer_id}")
            # WZ = tf.get_variable(name=f"dgcnn_WZ_{layer_id}", shape =[self.k, self.k])
            # WZ = self.weight_matrix(shape=(A_dim, B_dim), name=f"dgcnn_WZ_{layer_id}")
            # A = tf.multiply(self.ajacent, WZ)
            # AZ = tf.sparse_tensor_dense_matmul(A, input_Z)
            # AZ = tf.multiply(AZ, WZ)
    ...
    def output_layer(self, inputs):
        ...
        with tf.name_scope("output_layer"):
            ...
            outputs = tf.layers.dense(drop_out, self.hypers.class_num, activation=tf.nn.sigmoid)
        return outputs
